...
def make_real_space_embedding_model(detector_data, bpsplit_z, bpsplit_phi):
    '''
    ...
    * Callable
    ...
    bp_positons = np.vstack([
        np.resize([ 0.01*np.cos(a) for a in angles ], total_beampipe_split),
        np.resize([ 0.01*np.sin(a) for a in angles ], total_beampipe_split),
